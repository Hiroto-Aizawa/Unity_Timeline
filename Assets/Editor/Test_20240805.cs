...
public class FlameCheckTracksOrder : EditorWindow
{
    ...
    private int _prevObjectId = -1;
    private bool _isShowStageChildTrack = false;
    private bool _isFirstFindDecoration = true;
    ...
    private void CheckStageTrack(TrackAsset track)
    {
        ...
        if (childTracks != null)
        {
            List<string> childTrackList = childTracks.Select(e => e.name).ToList();

            // トラックの中にDecoraionがあれば、仕様リストにDecorationを追加する
            if (childTrackList.Contains("Decoration") && _isFirstFindDecoration)
            {
                _stageOrder.Add("Decoration");
                _isFirstFindDecoration = false;
            }
            else if (!childTrackList.Contains("Decoraion"))
            {
                _stageOrder.Remove("Decoration");
                _isFirstFindDecoration = true;
            }

            // childTracksから仕様に合っているトラックだけを抽出
            List<TrackAsset> regularTracks = childTracks.Where(e => _stageOrder.Contains(e.name)).ToList();

            if (regularTracks.Count == _stageOrder.Count && regularTracks.Count == childTracks.Count)
            {

                int i = 0;

                GUILayout.Label($"【ステージ】");
                EditorGUILayout.LabelField($"Detect Correct COunt");
                _isShowStageChildTrack = GUILayout.Toggle(_isShowStageChildTrack, "ステージの子トラックを表示する");
                GUILayout.Space(20);

                if (_isShowStageChildTrack)
                {
                    foreach (TrackAsset childTrack in childTracks)
                    {
